...
def load_config(config_file="config.json"):
    ...
    default_config = {
        "input_image": "pictures/hall.jpg",
        "output_image": "pictures/detected_hall.jpg",
    ...
    try:
        ...
        logging.error(
            f"Invalid JSON in config file '{config_file}'. Using default settings."
        )
        return default_config
...
def main():
    ...
    logger.info(
        f"Face detection process started at {start_time.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    ...
    # Log configuration
    logger.info(
        f"Configuration: {json.dumps({k: v for k, v in config.items() if k != 'logging'}, indent=2)}"
    )
    ...
    logger.info(
        f"Image enhancements: Histogram equalization={'Enabled' if use_histogram_equalization else 'Disabled'}, "
    ...
    try:
            ...
            try:
                ...
                if save_intermediate_images:
                    equalized_rgb = cv2.cvtColor(
                        cv2.merge([equalized_gray, equalized_gray, equalized_gray]),
                        cv2.COLOR_RGB2BGR,
                    )
                    equalized_output = (
                        f"pictures/equalized_{os.path.basename(input_image)}"
                    )
                    cv2.imwrite(equalized_output, equalized_rgb)
                    logger.info(
                        f"Saved histogram equalized image to: {equalized_output}"
                    )
        ...
        if use_gamma_correction:
            ...
            try:
                processed_image = apply_gamma_correction(
                    processed_image, gamma=gamma_value
                )
                logger.debug("Gamma correction completed")

                # Save intermediate result if requested
                if save_intermediate_images:
                    gamma_output = f"pictures/gamma_{os.path.basename(input_image)}"
                    cv2.imwrite(
                        gamma_output, cv2.cvtColor(processed_image, cv2.COLOR_RGB2BGR)
                    )
        ...
        face_locations = face_recognition.face_locations(
            processed_image, model=face_detection_model
        )
        detection_time = (datetime.datetime.now() - detection_start).total_seconds()
        logger.info(
            f"Found {len(face_locations)} faces in {detection_time:.2f} seconds"
        )
        ...
        for i, (top, right, bottom, left) in enumerate(face_locations):
            ...
            height = bottom - top
            cv2.rectangle(
                output_image_bgr, (left, top), (right, bottom), (0, 255, 0), 2
            )
            logger.debug(
                f"Face #{i+1}: position=({left},{top},{right},{bottom}), size={width}x{height} pixels"
            )
